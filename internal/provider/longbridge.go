...
func NewLongbridge() (*Longbridge, error) {
	...
	return &Longbridge{quotes: qc, news: cc, pacer: newNewsPacer(), close: qc.Close}, nil
}
...
// A bare `N` fans out one request per watchlist symbol; the pacer serializes that burst against
// Longbridge's minimum call interval and retries a rate-limit answer on a backoff ladder before it
// is allowed to surface as a 502.
func (l *Longbridge) News(ctx context.Context, symbol string, limit int) ([]NewsItem, error) {
	if symbol == "" {
		return nil, ErrSymbolRequired
	}
	var raw []*content.NewsItem
	err := l.pacer.fetch(ctx, func(ctx context.Context) error {
		var callErr error
		raw, callErr = l.news.News(ctx, symbol)
		return callErr
	})
	if err != nil {
